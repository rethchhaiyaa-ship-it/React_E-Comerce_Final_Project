...
export default function HomePage() {
  ...
  return (
    <div className="font-body">
      {/* ── HERO ── */}
      <section className="relative w-screen overflow-hidden min-h-[92vh] flex items-center">
        <div
        ...
        <div data-aos="fade-up" className="relative z-10 max-w-7xl mx-auto px-4 py-24 grid lg:grid-cols-2 gap-12 items-center">
          <div>
      ...
      {/* ── CATEGORIES ── */}
      <section data-aos="fade-up" className="max-w-7xl mx-auto px-4 py-16">
        ...
        <div data-aos="fade-up" className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
          {categories.map((cat) => (
            <Link
              key={cat.id}
      ...
      {/* ── POPULAR PRODUCTS ── */}
      <section data-aos="fade-up" className="bg-muted py-16">
      ...
      <section data-aos="fade-up" className="max-w-7xl mx-auto px-4 py-16">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
          {[
            { title: "Fast Delivery", desc: "Under 30 minutes, guaranteed or money back" },
            { title: "Fresh Ingredients", desc: "Every dish made fresh to your order, never frozen" },
            { title: "Secure Payment", desc: "Bank-level encryption on every transaction" },
            { title: "Easy Ordering", desc: "Order in 3 taps, track in real time" },
            { title: "24/7 Support", desc: "Real humans, always available, always helpful" },
          ].map((f) => (
      ...
      {/* ── APP BANNER ── */}
      <section data-aos="fade-up" className="max-w-7xl mx-auto px-4 pb-6">
        <div className="bg-charcoal rounded-3xl overflow-hidden relative">
          ...
          <div className="relative z-10 px-8 py-14 flex flex-col lg:flex-row items-center justify-between gap-8">
            ...
            <div className="flex flex-col sm:flex-row gap-3 shrink-0">
              {[
                { name: "App Store", tagline: "Download on the", icon: "/assets/icons/app-store.svg", alt: "App Store" },
                { name: "Google Play", tagline: "Download on", icon: "/assets/icons/google-play.svg", alt: "Google Play" },
              ].map((s) => (
                <button key={s.name} className="flex items-center gap-3 bg-white/10 hover:bg-white/20 border border-white/20 text-white px-5 py-3.5 rounded-xl transition-colors">
                  <img src={s.icon} alt={s.alt} className="h-6 w-auto shrink-0" />
                  <div className="text-left">
                    <p className="text-[10px] text-white/60">{s.tagline}</p>
                    <p className="font-800 text-sm">{s.name}</p>
                  </div>
                </button>
              ))}
            </div>
            ...
            {reviews.map((r) => (
              <div key={r.id} className="bg-card border border-border-custom rounded-2xl p-6 flex flex-col gap-4">
                ...
                <div className="flex items-center gap-3 pt-3 border-t border-border-custom">
                  <img
                    src={r.avatar.startsWith("http") ? r.avatar : `${IMG}${r.avatar}?w=80&h=80&fit=crop&auto=format`}
                    alt={r.name}
        ...
        <div className="bg-primary rounded-3xl px-8 py-14 text-center">
          ...
          >
            <input
              ...
              className="flex-1 bg-white text-charcoal placeholder:text-stone/60 px-5 py-3.5 rounded-xl text-sm focus:outline-none"
            />
            <button
              type="submit"
